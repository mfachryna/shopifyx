...
func (ph *ProductHandler) Create(ctx *fasthttp.RequestCtx) {
	...
	if !ok {
		response.Error(ctx, apierror.CustomServerError("userId not found in context"))
		return
	}
	uuid := uuid.New()
	...
	if _, err := ph.db.Exec(
		...
		fmt.Println(err)
		response.Error(ctx, apierror.CustomServerError("failed to insert data"))
		return
	}

	data.ID = uuid.String()

	response.Success(ctx, apisuccess.CustomResponse(
		http.StatusCreated,
